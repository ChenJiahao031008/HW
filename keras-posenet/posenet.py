...
def euc_loss3q(y_true, y_pred):
    lq = K.sqrt(K.sum(K.square(y_true[:,:] - y_pred[:,:]), axis=1, keepdims=True))
    return (1 * lq)
    # return (100 * lq) # when four param


def line_loss1(y_true, y_pred):
    r_true = (y_true[:,0:1])
    sin_true = y_true[:,1:2]
    cos_true = tf.sqrt(1-sin_true*sin_true)
    r_pred = (y_pred[:,0:1])
    sin_pred = y_pred[:,1:2]
    cos_pred = tf.sqrt(1-sin_pred*sin_pred)

    sin_delta = (sin_pred * cos_true - sin_true * cos_pred)
    cos_delta = cos_pred * cos_true + sin_true * sin_pred
    # loss_1 = tf.sqrt(K.sum(tf.square(r_pred - r_true*cos_delta), axis=-1, keepdims=True))
    # loss_2 = tf.sqrt(K.sum(tf.square(r_true - r_pred*cos_delta), axis=-1, keepdims=True))
    # loss = (loss_1 + loss_2)/2
    loss = tf.sqrt(K.sum(tf.square(r_pred)+ tf.square(r_true)- 2*(r_pred)*(r_true)*cos_delta))
    return (0.3 * loss)
...
def create_posenet(weights_path=None, tune=False):
    ...
    with tf.device('/cpu:0'):
        ...
        cls1_fc_pose_xyz = Dense(1,name='cls1_fc_pose_xyz')(cls1_fc1_pose)
        # cls1_fc_pose_xyz = Dense(2,name='cls1_fc_pose_xyz')(cls1_fc1_pose)

        cls1_fc_pose_wpqr = Dense(1,name='cls1_fc_pose_wpqr')(cls1_fc1_pose)
        # cls1_fc_pose_wpqr = Dense(2,name='cls1_fc_pose_wpqr')(cls1_fc1_pose)

        # cls1 = Dense(2,name='cls1')(cls1_fc1_pose)
        ...
        cls2_fc_pose_xyz = Dense(1,name='cls2_fc_pose_xyz')(cls2_fc1)
        # cls2_fc_pose_xyz = Dense(2,name='cls2_fc_pose_xyz')(cls2_fc1)

        cls2_fc_pose_wpqr = Dense(1,name='cls2_fc_pose_wpqr')(cls2_fc1)
        # cls2_fc_pose_wpqr = Dense(2,name='cls2_fc_pose_wpqr')(cls2_fc1)

        # cls2 = Dense(2,name='cls2')(cls2_fc1)
        ...
        cls3_fc_pose_xyz = Dense(1,name='cls3_fc_pose_xyz')(cls3_fc1_pose)
        # cls3_fc_pose_xyz = Dense(2,name='cls3_fc_pose_xyz')(cls3_fc1_pose)

        cls3_fc_pose_wpqr = Dense(1,name='cls3_fc_pose_wpqr')(cls3_fc1_pose)
        # cls3_fc_pose_wpqr = Dense(2,name='cls3_fc_pose_wpqr')(cls3_fc1_pose)

        # cls3 = Dense(2,name='cls3')(cls3_fc1_pose)
        ...
        # posenet = Model(input=input, output=[cls1, cls2, cls3])
